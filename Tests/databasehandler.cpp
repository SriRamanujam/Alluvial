...
bool DatabaseHandler::createTable()
{
    ...
    if(query.lastError().text() == "table Alluvial already exists Unable to execute statement"){
        table1 = true;
        ret = true;
    }
    query.clear();
    if (db.isOpen()){
        ret = query.exec("create table Playlist "
                          "(id integer primary key, "
                          "name varchar, "
                          "filepath varchar)");
    }
    //check to make sure tables created
    if(query.lastError().text() == "table Playlist already exists Unable to execute statement"){
        table2 = true;
        ret = true;
    }
    if(table1 && table2 == true){
        ret = true;
    }
    return ret;
}
